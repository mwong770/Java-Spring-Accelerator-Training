...
import com.company.mariawongu1capstone.exception.*;
...
public class ControllerExceptionHandler {
    ...
    // Handles invalid arguments from user such as invalid state or invalid item type
    ...
    // Handles insufficient inventory quantity in Invoice APIs
    @ExceptionHandler(value = {InsufficientQuantityException.class})
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public ResponseEntity<VndErrors> insufficientQuantityException(InsufficientQuantityException e, WebRequest request) {
        VndErrors error = new VndErrors(request.toString(), "Insufficient Quantity : " + e.getMessage());
        ResponseEntity<VndErrors> responseEntity = new ResponseEntity<>(error, HttpStatus.UNPROCESSABLE_ENTITY);
        return responseEntity;
    }

    // Handle sales that are too large for database in Invoice APIs
    @ExceptionHandler(value = {ValueNotSupportedException.class})
    @ResponseStatus(HttpStatus.UNPROCESSABLE_ENTITY)
    public ResponseEntity<VndErrors> valueNotSupportedException(ValueNotSupportedException e, WebRequest request) {
        VndErrors error = new VndErrors(request.toString(), "Sale Value Not Supported : " + e.getMessage());
        ResponseEntity<VndErrors> responseEntity = new ResponseEntity<>(error, HttpStatus.UNPROCESSABLE_ENTITY);
        return responseEntity;
    }
